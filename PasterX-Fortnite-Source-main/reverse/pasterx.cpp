...
{
	int isCached = ReadBizzy<int>(mesh + OFFSETS::BoneArrayCache);
	BoneArray = ReadBizzy<DWORD_PTR>(mesh + OFFSETS::BoneArray);
	if (BoneArray == NULL)
	{
		BoneArray = ReadBizzy<DWORD_PTR>(mesh + OFFSETS::BoneArray + 0x10);
	}
	return ReadBizzy<FTransform>(BoneArray + (index * 0x60));
}
...
{
	CameraInfo camera;
	auto location_pointer = ReadBizzy<uintptr_t>(Uworld + 0x110); //110
	auto rotation_pointer = ReadBizzy<uintptr_t>(Uworld + 0x120); //120
	...
	rotationInfo.pitch = ReadBizzy<double>(rotation_pointer);
	rotationInfo.yaw = ReadBizzy<double>(rotation_pointer + 0x20);
	rotationInfo.roll = ReadBizzy<double>(rotation_pointer + 0x1d0);
	...
	camera.fov = ReadBizzy<float>((uintptr_t)PlayerController + 0x394) * 90.f;
...

{
	...
	while (!driver.init()) {
		...
		std::count << "Driver is not loaded\n";
	}
	else
	{
		std::count << "Driver is loaded\n";
		}
